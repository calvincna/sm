...
if ($request_type == 'payments_get_items') {
  ...
  if ($item_id == '1a') {
    $item = array(
      'title' => 'Remove ads',
      'description' => 'Support Spotmania - ads will be removed and you get 1 additional clue every time you start a new game!',
      // Price must be denominated in credits.
      'price' => 25,
      'image_url' => 'http://www.playchap.com/spotmania/images/arrowup.gif',
    );
  ...
  if ($current_order_status == 'placed') {
	...
	$queryStr = "INSERT INTO removeads (fid,status,date,orderid) VALUES ('".$order_details['buyer']."',1,'".date('Y-m-d H:i:s')."','".$order_details['order_id']."')";
	...
	$queryStr = "UPDATE removeads SET status=3, date='".date('Y-m-d H:i:s')."' WHERE fid='".$order_details['buyer']."'";
	$result = mysql_query($queryStr) or die(mysql_error());	

  } else if ($current_order_status == 'refunded') {
    // Track refunded item orders initiated by Facebook. No need to respond.
	$queryStr = "UPDATE removeads SET status=0, date='".date('Y-m-d H:i:s')."' WHERE fid='".$order_details['buyer']."'";
	$result = mysql_query($queryStr) or die(mysql_error());	
