...
<body>
	...
	<div id="bannerhead" style="display:none"></div>
	<div id="bannerad1" style="display:none">
	</div>
	<div id="logo" style="display:none"></div>
	<div id="menu" class="menu1" style="display:none">
	...
	<div id="screenDiv">
		<div id="gameScreen">
			...
			<div class="clear"></div>
			<div id="result" style="display:block">test</div>
		...
		<div class="screen" id="startScreen" style="display:block">
			<div class="startTxt" style="display:none">SPOTMANIA!</div>
			<div class="startTxt">Find all 5 differences in the 2 pictures within the time limit. If you click on a wrong spot, you will lose some time.  Use a clue when you get stuck but do note that these are limited!</div>
					...
					<div class="startTxt" style="display:none">Tip: LIKE Spotmania to get an extra clue every time you start a new game (please refresh the game after you LIKE for the extra clue to take effect)</div>
	...
	<script>		
		...
		$(document).ready(function(){
			...
			var canvasLeft = parseInt($("#canvas1").offset().left);
			var canvasTop = parseInt($("#canvas1").offset().top);
			...
			//var gamePointer = 1; 
				...
				var encoded = encodeURI("type=" + gameType + "&stage="+ pad(arrGames[gameType][gamePointer], 3));
				
				//console.log("type=" + gameType + "&stage="+ pad(gamePointer, 3));
			...
			$("#canvas1").mouseup(function(e) {
				...
				(parseInt(e.pageY)-canvasTop) + " ::: " + $("#canvas1").width() + " , " + $("#canvas1").height());
				...
				var calpageX = ((parseInt(e.pageX)-canvasLeft) / ($("#canvas1").width())*imgWidth);
				var calpageY =  ((parseInt(e.pageY)-canvasTop) / ($("#canvas1").height())*imgHeight);
				
				 if (gameState==1) {
					//checkSpots(parseInt(e.pageX)-canvasLeft, parseInt(e.pageY)-canvasTop);
					checkSpots(calpageX, calpageY);
			...
			$("#canvas2").mouseup(function(e) {
				...
				//$("#result").html(String(parseInt(e.pageX)-canvasLeft-293)+", "+String(parseInt(e.pageY)-canvasTop));
			...
			//var canvasTop = parseInt($("#canvas1").offset().top);
				...
				var calpageX = ((parseInt(e.pageX)-parseInt($("#canvas2").offset().left)) / ($("#canvas2").width())*imgWidth);
				var calpageY =  ((parseInt(e.pageY)-parseInt($("#canvas2").offset().top)) / ($("#canvas2").height())*imgHeight);
			...
			function saveGame()
			{
				...
				//check if easy has reached max
				if ((gameType=="easy")&&(gameLevel<=easyLast)) {
					gameType="hard";
					gamePointer=1;
				} else if ((gameType=="hard")&&(gameLevel<=hardTotal)) {
					//gameover
					endGame();
				} else {
					gamePointer++;
